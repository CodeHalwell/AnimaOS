...
## Stage 7 — Autonomous Agent Layer (E7–E16)

The forward epics E7–E16 build the autonomy, embodiment, and alignment
layer on top of the shipped somatic core (Stages 1–6).  Epic numbers
E7–E16 continue the stable identifier sequence, and each epic references
its own design document in the companion design-doc suite (docs 12–21).
The forward-epic index, full dependency graph, and recommended build
sequence are recorded in `docs/18-forward-epics.md`.

Epics in this stage run as parallel tracks that converge at E12
(Motivation).  Stories within an epic may be parallelised, but an epic
closes only when all exit criteria are met.

**Sequencing note (E7/E8).** Give the cortex genuine ability to act on
the world (E7) and let operators plug in any local inference stack without
bespoke per-vendor code (E8).  E7 delivers the egress substrate, the first
real tools, and semantic tool selection; E8 delivers the OpenAI-compatible
umbrella that lights up five local providers at once and the `ChatBackend`
trait that E7 Phase 4 (live tool-calling) and E8 Phase 1 share.  E7 and E8
execute in parallel once the shared `LlmBackend` chat/tool-calling
extension lands (E8 S8.0); the extension adds backward-compatible default
methods so existing impls compile unchanged.
...
and live Anthropic/Ollama tool-calling.  All stories S7.0–S7.4 ✅.
See `docs/12-real-world-tools-plan.md`.

**Dependencies.** E5.1 (cortex/tool dispatch), E5.2 (gate), E5.3 (router),
E2.3 (praxis tool registry), E5.6 (motor gate), E3.3 (sensory bridge).

**Stories.**
- S7.0 Foundations: `crates/actuators` crate, async network substrate,
  `EgressGuard` (HTTPS-only, SSRF protection, domain allow/deny, rate limits),
  motor-gate hook at dispatch, env-var config + secret redaction. ✅ (PR #72 merged)
...
Native FFI runtimes, Unsloth as the default trainer, HRA for the instinct
tier, an eval harness, and an adapter library with dynamic mounting
complete the epic.  See `docs/13-local-llm-providers.md`.
...
guided journey: `anima doctor` preflight, `anima init` wizard, conversational
identity bootstrap, non-NVIDIA/CPU/Apple Silicon support, per-tier router
dispatch, and a unified quickstart document.
...
**Remaining.** S9.6 ⬜ (bare-metal runbook — external).  An earlier status
snapshot also listed S9.4 🟡 ("Docker profile deferred"); S9.4 has since
closed via the CPU/Apple-Silicon compose overlays and the GHCR image
workflow recorded under S9.4 above.

### Epic E10 — Presence ✅

**Scope.** Channel gateway framework: comms-app adapters (Telegram/Slack
first) over the existing operator seam; text, image, and voice as
first-class bidirectional modalities.
...
audit entries).  See `docs/17-motivation-and-drives.md`.
...
**Scope.** Immutable value charter; constitution-enforcement hook; continuous
...
**Delivered (PR #75).** All stories S13.1–S13.6 ✅; 31 unit tests.
...
corpus (RAG), and (4) cognitive watchdogs for stuck-loop detection with
agent-level rollback.  See `docs/20-higher-cognition.md`.
...
migration.  The approval queue is the operator-facing half of E11's
promotion gate.  See `docs/21-operator-trust-and-lifecycle.md`.
...
The following decisions gated Stage 5 epics; all are now resolved — the
records are retained for traceability.  They mirror Section 13 of
...
**Resolution.** Resolved as the working assumption — hosted-daemon for the
E5.x cognitive layer, microVM for the production target; the
pre-integrated-distribution path remains explicitly out of scope.
...
**Resolution.** Resolved — local-first default routing shipped in the
Thalamic Router (`StaticRouter` routes cheap-local by default; API tiers
require explicit provider keys/opt-in).  Real-workload validation continues
as part of the (open) hardware-gated tails.
...
**Resolution.** Resolved — `kv-controller` ships `TraceProvenance`
`{live, synthetic, public}`: synthetic generation plus live trace capture
feed the offline training pipeline; the needle-recall benchmark vs LRU is
the eval (E5.4 closed on this basis).
...
**Resolution.** Resolved — the operator console (Epic E6: HTTP/SSE
dashboard + `anima-console` TUI/CLI) plus the `anima` CLI subcommands are
the user-facing surface; E10 comms gateways extend it.
...
**Resolution.** Resolved — the privacy/trust story shipped as E17 (per-user
identity, trust tiers, consent model) and E23 (consent enforcement & data
lifecycle); the conservative opt-in posture documented here was retained.
...
**Resolution.** Resolved as path (a) — custom TurboQuant in Rust over the
existing `L3Archive` (E2.7); the LanceDB/Qdrant swap was not needed and the
no-host-OS posture is preserved.
...
## Stage 9 — Operational Wave & Multi-Tenancy (E18–E32)

The operational wave on top of the E1–E17 core: rate limiting,
observability, configuration, sessions, data governance, performance,
and multi-tenancy.  E18–E30 were developed as fourteen independent
feature branches and integrated as a single coherent line — the
integration notes at the end of this stage record the decisions made at
that merge; E31 and E32 extend the wave with workspace management and
job scheduling.  Epics are ordered numerically below.
...
### Observability, Knowledge & Analytics
...
### Epic E32 — Scheduled Job and Cron Engine ✅
