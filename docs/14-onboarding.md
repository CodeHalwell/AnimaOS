...
> **Status:** Proposed (scoping). Target epic: **E8 — Onboarding**.
> Branch: `claude/llm-tools-animaos-vuXRK`.
> Related: E6 (tools), E7 (providers), E9 (communication), E10 (skills).
...
## 2. Workstreams — Epic E8, stories `S8.x`

### S8.1 — Guided first-run / `anima init` wizard *(gap 1)*
...
### S8.2 — Conversational identity bootstrap *(gap 2)*
...
- Instead of `identity set k v`, the agent **interviews** the new user on first
  ...
  preferred comms channel (feeds E9). Answers populate `IdentityMemory` via the
  existing `set_fact` path (so the audit trail is identical).
- Implemented as a built-in onboarding **skill** (depends on E10) once the live
  cortex (E6 S6.4) can hold a conversation; until then, a scripted fallback
  asks the same questions deterministically.
- Identity write-back is gated and audited exactly as manual edits are.

### S8.3 — Preflight & hardware/provider detection *(gap 4)*
...
- `anima doctor`: detects GPU vendor/VRAM (NVIDIA via `nvidia-smi`, Apple
  ...
  providers (probe Ollama/LM Studio/vLLM ports — reuse E7 health probes), and
  RAM/disk headroom.
- Emits a capability profile that the wizard (S8.1) uses to **recommend** a
...
### S8.4 — Non-NVIDIA / CPU / Apple-Silicon support *(gap 4)*
...
- This is mostly packaging + docs + E7 provider presets; the cognitive code is
  already backend-agnostic at the `LlmBackend` trait.

### S8.5 — Per-tier router dispatch *(gap 3)*
...
- This is the `docker/README.md` limitation #1 / `docs/10 §"LifecycleManager is
  ...
  "pick a model per tier" step to mean anything. Shared with E7 §4.

### S8.6 — Bare-metal onboarding story *(gap 5)*
...
anima init              → pick providers per tier (recommended defaults pre-filled)
                          ...
                          → "I'm awake. Here's how to reach me." (hands off to E9)
open console / channel   → ongoing interaction
```

## 4. Cross-cutting & dependencies

- **E7 S7.0** health probes power S8.3; **E7 §4 / S8.5** tier→backend map is
  shared. **E10** skills enable the conversational bootstrap (S8.2).
...
- How much of S8.2 waits on the live cortex vs ships scripted now.
