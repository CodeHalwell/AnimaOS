...
impl TrafficKeys {
    ...
    fn seal(&mut self, header: &[u8; 5], inner: &mut Vec<u8>) {
        ...
        let mut cipher = Aes128Gcm::new_from_slice(&self.key).expect("key must be 16 bytes");
        let tag = cipher
...
pub fn run_tls_loopback_test() -> Result<(), &'static str> {
    ...
    let (_, cfin_record) = read_record(&c2s, &mut c2s_pos)?;
    if cfin_record.is_empty() || cfin_record[0] != CT_APP_DATA {
        return Err("server: expected encrypted client Finished");
    }
    ...
    let _inner_ct = cfin_ct.pop(); // should be CT_HANDSHAKE (22)
    ...
    let (_, ping_record) = read_record(&c2s, &mut c2s_pos)?;
    if ping_record.is_empty() || ping_record[0] != CT_APP_DATA {
        return Err("server: expected encrypted PING record");
    }
    ...
    ping_ct.pop(); // remove CT_APP_DATA inner byte
...
///
/// Verifies that the decrypted inner content type is `CT_HANDSHAKE` (22)
/// and that the handshake message type byte matches `expected_type`.
