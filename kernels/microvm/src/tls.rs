...
impl RdRand {
    /// Draw a single 32-bit value from the x86 RDRAND instruction.
    ///
    /// Intel's RDRAND guidance recommends retrying up to 10 times.  A
    /// persistent failure after 10 retries indicates a hardware fault or a
    /// hypervisor that does not implement RDRAND; we panic rather than spinning
    /// indefinitely, which would hang the bare-metal kernel.
    fn rdrand_u32(&mut self) -> u32 {
        for _ in 0..10 {
            let mut val: u32;
            let ok: u8;
...
// rand_core 0.10 makes the infallible `Rng` (and `CryptoRng`) traits blanket
// extensions of `TryRng<Error = Infallible>` (and `TryCryptoRng`).  We therefore
// implement the fallible traits — RDRAND never fails (it panics on hardware
// fault), so the error type is `Infallible` — and the infallible `Rng` /
// `CryptoRng` impls are derived automatically by rand_core's blanket impls.
impl rand_core::TryRng for RdRand {
    ...
    fn try_next_u64(&mut self) -> Result<u64, Self::Error> {
        Ok(((self.rdrand_u32() as u64) << 32) | (self.rdrand_u32() as u64))
    }

    fn try_fill_bytes(&mut self, dest: &mut [u8]) -> Result<(), Self::Error> {
        for chunk in dest.chunks_mut(4) {
            let v = self.rdrand_u32().to_le_bytes();
...
impl rand_core::TryCryptoRng for RdRand {}
...
fn build_client_hello(client_random: &[u8; 32], client_public_key: &PublicKey) -> Vec<u8> {
    ...
    // key_share (0x0033): one P-256 key share
    {
        ...
        let pk_point = client_public_key.to_sec1_point(false);
...
fn build_server_hello(server_random: &[u8; 32], server_public_key: &PublicKey) -> Vec<u8> {
    ...
    {
        let pk_point = server_public_key.to_sec1_point(false);
        let pk_bytes = pk_point.as_bytes();
...
pub fn run_tls_loopback_test() -> Result<(), &'static str> {
    ...
    // ----------------------------------------------------------------
    let mut rng = RdRand;
    let client_secret = EphemeralSecret::generate_from_rng(&mut rng);
    let client_pk = client_secret.public_key();

    let server_secret = EphemeralSecret::generate_from_rng(&mut rng);
...
pub fn run_tls_over_smoltcp_test() -> Result<(), &'static str> {
    ...
    if client_pk_parsed.to_sec1_point(false).as_bytes() != client_pk.to_sec1_point(false).as_bytes()
    {
