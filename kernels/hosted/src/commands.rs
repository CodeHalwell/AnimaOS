//! Subcommand handlers for the `anima-hosted` CLI (KERN-9).
//!
//! Each `cmd_*` function implements one top-level subcommand that `main`
//! routes to via a single `match`, exiting with the status recorded through
//! [`crate::cli_fail`]. Splitting these handlers out of `main.rs` keeps the
//! crate root a thin entry point (panic hook, backend selection, dispatch,
//! and the two-agent demo) rather than a multi-thousand-line monolith.
//!
//! `use super::*` pulls in the crate-root helpers (`cli_fail`, `block_on`,
//! `flag`-parsing, the shared `use` re-exports, and the `audit_view` /
//! `cortex` sibling modules) that these handlers were written against.
...
/// user registry.
pub(crate) fn cmd_quota(args: &[String]) {
...
pub(crate) fn cmd_config(args: &[String]) {
...
/// ```
pub(crate) fn cmd_sessions(args: &[String]) {
    use sessions::{
            ...
            Some(user_id) => {
                ...
                match store.insert(session) {
                    Ok(()) => {
                        ...
                        audit_view::print_session_audit(&log);
                    }
                    Err(e) => {
...
pub(crate) fn cmd_data(args: &[String]) {
            ...
            Some(user_id) => {
                ...
                match registry.get(user_id) {
                    Some(rec) => {
                        ...
                        audit_view::print_data_audit(&log);
                    }
                    None => {
                        eprintln!("data: no user with id={user_id:?}");
                        cli_fail(1);
...
pub(crate) fn cmd_feedback(args: &[String]) {
    use std::str::FromStr;
...
/// ```
pub(crate) fn cmd_stats(args: &[String]) {
...
/// ```
pub(crate) fn cmd_cache(args: &[String]) {
...
/// ```
pub(crate) fn cmd_graph(args: &[String]) {
    const AGENT_ID: &str = "anima";
...
/// ```
pub(crate) fn cmd_metrics(args: &[String]) {
    const AGENT_ID: &str = "anima";
...
pub(crate) fn cmd_alert(args: &[String]) {
    const AGENT_ID: &str = "anima";
        ...
        "add" => {
            ...
            match registry.add(rule) {
                Ok(()) => {
                    ...
                    println!("\nAudit trail:");
                    audit_view::print_audit_alert(&log);
                }
                Err(e) => {
                    eprintln!("Error: {e}");
                    cli_fail(1);
...
/// ```
pub(crate) fn cmd_webhook(args: &[String]) {
    use webhooks::{
...
pub(crate) fn cmd_diagnose(args: &[String]) {
...
pub(crate) fn cmd_identity(args: &[String]) {
    const AGENT_ID: &str = "anima";
...
/// ```
pub(crate) fn cmd_users(args: &[String]) {
    use std::str::FromStr;
            ...
            (Some(user_id), Some(tier_str)) => match TrustTier::from_str(tier_str) {
                Ok(tier) => match registry.set_trust(user_id, tier, 0) {
                    Ok((old, new)) => {
                        ...
                        audit_view::print_user_audit(&log);
                    }
                    Err(e) => {
                        eprintln!("users: {e}");
                        cli_fail(1);
...
/// ```
pub(crate) fn cmd_workspace(args: &[String]) {
    use std::str::FromStr;
            ...
            (Some(raw_id), Some(display_name), Some(owner_user_id)) => {
                ...
                match registry.create(profile, 0) {
                    Ok(()) => {
                        ...
                        audit_view::print_workspace_audit(&log);
                    }
                    Err(e) => {
...
/// ```
pub(crate) fn cmd_jobs(args: &[String]) {
    use jobs::{
        ...
        Some("add") => {
            ...
            match registry.add(job) {
                Ok(()) => {
                    ...
                    audit_view::print_jobs_audit(&log);
                }
                Err(e) => {
                    eprintln!("jobs: {e}");
                    cli_fail(1);
...
pub(crate) fn cmd_skills(args: &[String]) {
    use skills::{
...
pub(crate) fn cmd_tools(args: &[String]) {
    use praxis::{Bus, ToolEnvelope};
...
pub(crate) fn cmd_skills_approval(args: &[String], log: &mut AuditLog) {
    use lifecycle::approval::ApprovalQueue;
    use lifecycle::skill_bridge::SkillApprovalBridge;
    use skills::{
...
pub(crate) fn cmd_ask(args: &[String]) {
...
pub(crate) fn cmd_digest(args: &[String]) {
...
/// ```
pub(crate) fn cmd_replay(args: &[String]) {
